...
public class purchase_servlet extends HttpServlet {
	...
	private void insertUser(HttpServletRequest request, HttpServletResponse response) 
		...
		purchase newUser = new purchase(product_id,suppid,date,quan,up,stu);
	...
	private void updateUser(HttpServletRequest request, HttpServletResponse response) 
		...
		float up = Float.parseFloat(request.getParameter("add2"));
		String stu = request.getParameter("status");
		purchase pur = new purchase(id,product_id,suppid,date,quan,up,stu);
		userDAO.updateUser(pur);
		response.sendRedirect("list1");
	}
	...
	private void deleteUser(HttpServletRequest request, HttpServletResponse response) 
		...
		boolean istrue = userDAO.deleteUser(id);
		if(istrue == true) {
			out.println("<script type = 'text/javascript'>");
			out.println ("alert('are you sure you want to cancel the order');");
			out.println("location ='list1'");
			out.println("</script>");
		}}
